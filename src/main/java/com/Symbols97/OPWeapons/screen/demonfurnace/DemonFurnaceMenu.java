...
public class DemonFurnaceMenu extends AbstractContainerMenu {
	
    public DemonFurnaceBlockEntity blockEntity;
    private Level level;
    private final ContainerData data;

    public DemonFurnaceMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
        this(pContainerId, inv, inv.player.level.getBlockEntity(extraData.readBlockPos()), new SimpleContainerData(4));
    }

    
    public DemonFurnaceMenu(int pContainerId, Inventory inv, BlockEntity entity , ContainerData data) {
        super(MenuTypes.demonfurnace_menu.get(), pContainerId);
        checkContainerSize(inv, 3);
        blockEntity = (DemonFurnaceBlockEntity) entity;
        ...
        this.blockEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY).ifPresent(handler -> {
            ...
            this.addSlot(new OPWResultSlot(handler, 2, 116, 35));
        });
    ...
    public boolean stillValid(Player pPlayer) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()),
                pPlayer, OPWBlocks.demon_furnace.get());
    }
