package com.Symbols97.OPWeapons.screen.opcraftingstation;
...
import com.Symbols97.OPWeapons.screen.resultslot.OPCSResultSlot;
...
public class OPCraftingStationMenu extends RecipeBookMenu<CraftingContainer> {
	   ...
	   public OPCraftingStationMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
			this(pContainerId, inv);
		}
	   
	   public OPCraftingStationMenu(int p_39353_, Inventory p_39354_) {
	      this(p_39353_, p_39354_, ContainerLevelAccess.NULL);
	   }

	   public OPCraftingStationMenu(int p_39356_, Inventory p_39357_, ContainerLevelAccess p_39358_) {
	      super(MenuTypes.opcs_menu.get(), p_39356_);
	      this.access = p_39358_;
	      this.player = p_39357_.player;
	      this.addSlot(new OPCSResultSlot(p_39357_.player, this.craftSlots, this.resultSlots, 0, 124, 35));
	   ...
	   protected static void slotChangedCraftingGrid(AbstractContainerMenu abContainer, Level level, Player player, CraftingContainer container, ResultContainer result) {
	      if (!level.isClientSide) {
	         ...
	         Optional<OPCSRecipe> optional = level.getServer().getRecipeManager().getRecipeFor(OPCSRecipe.Type.INSTANCE, container, level);
	         if (optional.isPresent()) {
	        	 OPCSRecipe craftingrecipe = optional.get();
	   ...
	   public boolean stillValid(Player player) {
	      return stillValid(this.access, player, OPWBlocks.op_crafting_station.get());
	   }
	   ...
	   public int getSize() {
	      return (OPCraftingStationMenu.width * OPCraftingStationMenu.height) + 1;
	   }
