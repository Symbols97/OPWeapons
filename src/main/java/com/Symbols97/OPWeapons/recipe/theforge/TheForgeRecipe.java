package com.Symbols97.OPWeapons.recipe.theforge;
...
public class TheForgeRecipe implements Recipe<CraftingContainer> {
	...
	public TheForgeRecipe(ResourceLocation id, String group, int width, int height,
	...
	public static class Type implements RecipeType<TheForgeRecipe> {
		private Type() {
		}

		public static final Type INSTANCE = new Type();
		public static final String ID = "the_forge";
	}

	static NonNullList<Ingredient> dissolvePattern(String[] name, Map<String, Ingredient> map, int intX, int intY) {
	...
	static String[] patternFromJson(JsonArray array) {
		...
		if (astring.length > TheForgeMenu.height) {
			throw new JsonSyntaxException("Invalid pattern: too many rows, " + TheForgeMenu.height + " is maximum");
		} else if (astring.length == 0) {
			throw new JsonSyntaxException("Invalid pattern: empty pattern not allowed");
		} else {
			for (int i = 0; i < astring.length; ++i) {
				String s = GsonHelper.convertToString(array.get(i), "pattern[" + i + "]");
				if (s.length() > TheForgeMenu.width) {
					throw new JsonSyntaxException("Invalid pattern: too many columns, " + TheForgeMenu.width + " is maximum");
				}
	...
	public static class Serializer implements RecipeSerializer<TheForgeRecipe> {
		...
		public static final ResourceLocation ID = new ResourceLocation(OPWeapons.MOD_ID, "the_forge");

		@Override
		public TheForgeRecipe fromJson(ResourceLocation id, JsonObject json) {
			String s = GsonHelper.getAsString(json, "group", "");
			Map<String, Ingredient> ingredientMap = TheForgeRecipe.keyFromJson(GsonHelper.getAsJsonObject(json, "key"));
			String[] patternArray = TheForgeRecipe.shrink(patternFromJson(GsonHelper.getAsJsonArray(json, "pattern")));
			int i = patternArray[0].length();
			int j = patternArray.length;
			NonNullList<Ingredient> dissolvePattern = TheForgeRecipe.dissolvePattern(patternArray, ingredientMap, i, j);
			ItemStack output = TheForgeRecipe.itemStackFromJson(GsonHelper.getAsJsonObject(json, "result"));

			return new TheForgeRecipe(id, s, i, j, dissolvePattern, output);
		}

		@Override
		public TheForgeRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buf) {
			...
			return new TheForgeRecipe(id, s, i, j, nonnulllist, itemstack);
		}

		@Override
		public void toNetwork(FriendlyByteBuf buf, TheForgeRecipe recipe) {
