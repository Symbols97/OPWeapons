...
public class WorldGenDemonTree extends WorldGenAbstractTree {

	public WorldGenDemonTree() {
		super(false);
	...
	public boolean generate(World world, Random random, int x, int y, int z) {
		...
		if (block != OPWeapons.blockofdemongem) {
			return false;
		} else {
			for (int i = -2; i <= 2; i++) {
				for (int j = -2; j <= 2; j++) {
			...
			for (int i = 0; i < baseLength; i++) {
				buildBlock(world, x, y + h, z, OPWeapons.OPWLogBlocks, 0);
				h++;
			}
	...
	public void generateTop(World world, Random random, int x, int y, int z) {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				buildBlock(world, x + i, y, z + j, OPWeapons.OPWLeafBlocks, 0);
		...
		buildBlock(world, x, y + 1, z, OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x + 1, y + 1, z, OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x, y + 1, z + 1, OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x - 1, y + 1, z, OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x, y + 1, z - 1, OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x + 2, y, z, OPWeapons.OPWLeafBlocks, 0);
	...
	public void generateBranchMid(World world, Random random, int x, int y, int z, int p) {
		...
		buildBlock(world, x, y , z, OPWeapons.OPWLogBlocks, 0);
		buildBlock(world, x, y + 1 , z, OPWeapons.OPWLogBlocks, 0);
	...
	public void generateBranchBase(World world, Random random, int x, int y, int z, int p) {
		...
		buildBlock(world, x - 2, y, z - 2, Blocks.air, 0);
		buildBlock(world, x, y, z - 3, OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x - 3, y, z , OPWeapons.OPWLeafBlocks, 0);
		
		buildBlock(world, x, y, z + 2 , OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x + 1, y, z + 2 , OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x - 1, y, z + 2 , OPWeapons.OPWLeafBlocks, 0);
		buildBlock(world, x, y, z + 3 , OPWeapons.OPWLeafBlocks, 0);
