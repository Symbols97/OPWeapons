package com.Symbols97.OPWeapons.projectiles;
...
public class EntityLapizArrow extends EntityArrow {
	private int xTile = -1;
	private int yTile = -1;
	private int zTile = -1;
	...
	private double damage = 9.5D;
	
	
	public EntityLapizArrow(World world) {
		super(world);
	}

	public EntityLapizArrow(World world, double x, double y, double z) {
		super(world, x, y, z);
	}
	public EntityLapizArrow(World world, double x, double y, double z, double damage) {
		super(world, x, y, z);
		//setDamage(damage);
	}

	public EntityLapizArrow(World world, EntityLivingBase shooter, EntityLivingBase target, float power, float variance) {
		super(world, shooter, target, power, variance);
	}

	public EntityLapizArrow(World world, EntityLivingBase shooter, EntityLivingBase target, float power, float variance, double damage) {
		super(world, shooter, target, power, variance);
		//setDamage(damage);
	}

	public EntityLapizArrow(World world, EntityLivingBase shooter, float power) {
		super(world, shooter, power);
	}

	public EntityLapizArrow(World world, EntityLivingBase shooter, float power, double damage) {
		super(world, shooter, power);
		//setDamage(damage);
	}
	...
	public void onCollideWithPlayer(EntityPlayer par1EntityPlayer)
    {
        if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0)
        {
            ...
            if (this.canBePickedUp == 1 && !par1EntityPlayer.inventory.addItemStackToInventory(new ItemStack(OPWeapons.lapizArrow, 1)))
            {
                flag = false;
            }
