...
public class OPBow extends BowItem{

	public OPBow(Properties p_40660_) {
		super(p_40660_);
	      ...
	      if (p_40669_ instanceof Player) {
	         ...
	         if (!itemstack.isEmpty() || flag) {
	            if (itemstack.isEmpty()) {
	               itemstack = new ItemStack(OPWItems.lapiz_arrow.get());
	            }

	            float f = getPowerForTime(i);
	            if (!((double)f < 0.1D)) {
	               boolean flag1 = player.getAbilities().instabuild || (itemstack.getItem() instanceof LapizArrowItem && ((LapizArrowItem)itemstack.getItem()).isInfinite(itemstack, p_40667_, player));
	               if (!p_40668_.isClientSide) {
	            	   LapizArrowItem LapizArrowItem = (LapizArrowItem)(itemstack.getItem() instanceof LapizArrowItem ? itemstack.getItem() : OPWItems.lapiz_arrow.get());
	                  AbstractArrow abstractarrow = LapizArrowItem.createArrow(p_40668_, itemstack, player);
...
//
	                 ...
	                 // System.out.println(abstractarrow);
	               }
	 ...
	 public static final Predicate<ItemStack> lapiz_arrow = (itemstack) -> {
	      return itemstack.is(OPWItems.lapiz_arrow.get());
	   };
